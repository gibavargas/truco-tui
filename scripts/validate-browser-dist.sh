...
./index.html
EOF
(cd "$DIST_DIR" && find . -type f ! -name 'truco-api' ! -name 'truco-api.exe' | sed -E "s/app\.[a-f0-9]+\.(css|js)/app.\1/" | sort) >"$tmp_actual"
...
if command -v curl >/dev/null 2>&1 && [[ -x "$api_bin" ]]; then
  ...
  css_file=$(ls "$DIST_DIR/assets" | grep "\.css$"); curl -fsS "http://127.0.0.1:${port}/assets/${css_file}" >/dev/null
  js_file=$(ls "$DIST_DIR/assets" | grep "\.js$"); curl -fsS "http://127.0.0.1:${port}/assets/${js_file}" >/dev/null
