...
./index.html
EOF
(cd "$DIST_DIR" && find . -type f ! -name 'truco-api' ! -name 'truco-api.exe' | sed -E 's/app\.[0-9a-f]+\.css/app.css/' | sed -E 's/app\.[0-9a-f]+\.js/app.js/' | sort) >"$tmp_actual"
...
if command -v curl >/dev/null 2>&1 && [[ -x "$api_bin" ]]; then
  ...
  css_file=$(find "$DIST_DIR/assets" -type f -name 'app.*.css' -exec basename {} \;)
  js_file=$(find "$DIST_DIR/assets" -type f -name 'app.*.js' -exec basename {} \;)
