...
# normalize the hashes from the actual files to match our expected template for testing
(cd "$DIST_DIR" && find . -type f ! -name 'truco-api' ! -name 'truco-api.exe' | sed -E 's/\.[0-9a-f]{8}\.(js|css)$/.\1/' | sort) >"$tmp_actual"
