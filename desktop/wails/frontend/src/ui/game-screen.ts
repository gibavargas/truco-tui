    ...
    case "chat":
      return `
        <div class="game10-panel">
          ...
          ${isOnlineMode ? `<form class="chat-form" data-api-action="sendChat" data-form-id="sendChat"><input name="message" aria-label="${escapeHtml(t('chat_placeholder'))}" type="text" autocomplete="off" placeholder="${escapeHtml(t("chat_placeholder"))}"><button class="secondary-button" type="submit"${busyAttr("sendChat")}>${buttonLabel("sendChat", t("lobby_chat"))}</button></form>` : ""}
        </div>
      `;
    default:
      return `
