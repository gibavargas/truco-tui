    ...
    case "chat":
      return `
        <div class="game10-panel">
          ...
          ${isOnlineMode ? `<form class="chat-form" data-api-action="sendChat" data-form-id="sendChat"><input name="message" type="text" autocomplete="off" aria-label="${escapeHtml(t('chat_placeholder'))}" placeholder="${escapeHtml(t("chat_placeholder"))}"><button class="secondary-button" type="submit"${busyAttr("sendChat")}>${buttonLabel("sendChat", t("lobby_chat"))}</button></form>` : ""}
        </div>
      `;
    default:
      return `
