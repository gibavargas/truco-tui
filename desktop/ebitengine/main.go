...
func (g *Game) Draw(screen *ebiten.Image) {
	...
	vector.FillRect(screen, 10, 10, screenWidth-20, screenHeight-20, color.RGBA{30, 110, 30, 255}, false) // nolint:staticcheck
...
func (g *Game) drawCard(screen *ebiten.Image, x, y int, c truco.Card, label string, isHovered bool) {
	...
	// Sombra
	vector.FillRect(screen, fx+4, fy+4, cardWidth, cardHeight, color.RGBA{0, 0, 0, 100}, false) // nolint:staticcheck
	// Fundo da carta
	vector.FillRect(screen, fx, fy, cardWidth, cardHeight, color.White, false) // nolint:staticcheck
	// Borda
	...
	vector.FillCircle(screen, fx+cardWidth-15, fy+15, 6, suitColor, true) // nolint:staticcheck
}
...
func (g *Game) drawOpponents(screen *ebiten.Image) {
	...
	for i := 0; i < p2HandSize; i++ {
		vector.FillRect(screen, float32(startX+i*40), 30, 35, 50, color.RGBA{150, 50, 50, 255}, false) // nolint:staticcheck
	}

	// Adversários Laterais (P1 e P3) - Simplificado
	vector.FillRect(screen, 30, float32(screenHeight/2-40), 40, 60, color.RGBA{50, 50, 150, 255}, false) // nolint:staticcheck
	vector.FillRect(screen, float32(screenWidth-70), float32(screenHeight/2-40), 40, 60, color.RGBA{50, 50, 150, 255}, false) // nolint:staticcheck
}
...
func main() {
	...
	logic, err := truco.NewGame([]string{"P1", "P2", "P3", "P4"}, []bool{false, false, false, false})
	if err != nil {
		log.Fatal(err)
	}
